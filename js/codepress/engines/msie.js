...
CodePress = {
	...
	metaHandler : function(ev) {
		keyCode = ev.keyCode;
		var Ret=String.fromCharCode(keyCode);var ret=Ret.toLowerCase()
		
		if(keyCode==9 || ev.tabKey) { 
			CodePress.snippets();
		}else if(ev.ctrlKey&&ret=="s"){	// ctrl s added by OOPUB
			ev.cancelBubble=true
			top.cms.EditSave()
		}else if((keyCode==122||keyCode==121||keyCode==90) && ev.ctrlKey) { // undo and redo
			(keyCode==121||ev.shiftKey) ? CodePress.actions.redo() :  CodePress.actions.undo(); 
		...
		}else if((ev.ctrlKey || ev.metaKey) && ev.shiftKey && keyCode!=90)  { // shortcuts = ctrl||appleKey+shift+key!=z(undo) 
			CodePress.shortcuts(keyCode);
			ev.returnValue = false;
		}else if(keyCode==86 && ev.ctrlKey)  { // handle paste
		...
		}else if(keyCode==67 && ev.ctrlKey)  { // handle cut
