...
CodePress = {
	...
	keyHandler : function(ev) {
		...
		if((ev.ctrlKey || ev.metaKey) && ev.shiftKey && charCode!=90)  { // shortcuts = ctrl||appleKey+shift+key!=z(undo) 
			CodePress.shortcuts(charCode?charCode:keyCode);
		}
//added by cms
		else if(ev.ctrlKey&&charCode==115){// ctrl s
			ev.preventDefault();
			ev.stopPropagation()
			top.cms.EditSave()
		}
		else if( (completeEndingChars.indexOf('|'+fromChar+'|')!= -1 || completeChars.indexOf('|'+fromChar+'|')!=-1) && CodePress.autocomplete) { // auto complete
			if(!CodePress.completeEnding(fromChar))CodePress.complete(fromChar);
		}
...
//modified by cms
		else if(keyCode==9 || ev.tabKey) {  // snippets activation (tab)
			//CodePress.snippets(ev)
			ev.preventDefault();
			ev.stopPropagation()
		...
		else if((charCode==122||charCode==121||charCode==90) && ev.ctrlKey) { // undo and redo
			(charCode==121||ev.shiftKey) ? CodePress.actions.redo() :  CodePress.actions.undo(); 
			ev.preventDefault();
		}
		else if(charCode==118 && ev.ctrlKey)  { // handle paste
		 	top.setTimeout(function(){CodePress.syntaxHighlight('generic');},100);
		}
		else if(charCode==99 && ev.ctrlKey)  { // handle cut
	...
	snippets : function(ev) {
		...
		for (var i=0; i<snippets.length; i++) {
			if(snippets[i].input == trigger) {
				...
				ev.preventDefault(); // prevent the tab key from being added
...
// rajouter par cms
		try{
			if(range.startContainer.tagName.toLowerCase()=="body")
				range.setStartBefore(window.document.getElementsByTagName('pre')[0].childNodes[0],0);
		}catch(e){}
	...
	setCode : function() {
		...
		if (code == '')document.getElementsByTagName('body')[0].innerHTML = '';
	},
